...
// import { UserProfilePhoto } from '/imports/dashboard/list_of_users/userProfilePhotoServer/userPrifilePhoto.js';
// import { SuperUser } from '/imports/dashboard/commonAPI/SuperUser.js';
// if (Meteor.isServer) {
...
//     Counts.publish(this,"students-count",Meteor.users.find({"roles": "Customer"}));
//   });
...
//   },
  ...
  // UMupdateUserDetailsByAdmin:function(formValues){
  //   Meteor.users.update(
  //     {'_id': formValues.userId},
  //     {
  //       $set: {
  //           "profile.fullname" : formValues.adminEditUserFullName,
  //           "profile.mobNumber" : formValues.adminEditUserMobile,
  //       }
  ...
  // addUserImageFromS3ToProfile:function(userId, s3Id){
  //   var data = UserProfilePhoto.findOne({"_id":s3Id});
  ...
  addUserAddress:function(formValues,userId){
  ...
  'editDeliveryAddress': function(formValues,userId,index){
          var userData = Meteor.users.findOne({"_id":userId});
          var newArr = [];
          if(userData){
            newArr = userData.profile.deliveryAdd;
  ...
  deleteDeliveryAddress:function (currentIndex ,userId) {
    Meteor.users.update(
      {'_id':userId},
      { 
          $unset:{ 

            ["profile.deliveryAdd."+currentIndex]:1,
   ...
   removenullAddress:function(userId) {

       Meteor.users.update(
        {'_id':userId},
        { 
          $pull:{
                "profile.deliveryAdd":null,
          }
        }
      )
   },
...
//       var getSuperUser = SuperUser.findOne({"_id":"1"});
     
//       superUserId = Accounts.createUser({
//                 // username : getSuperUser.username,
//                 // email    : getSuperUser.email,
//                 // password : getSuperUser.password,
//                 // profile  : getSuperUser.profile,
//                  username : "superAdmin",
...
//   process.env.MAIL_URL = "smtp://iassureit443@gmail.com:iAssureIT\@123@smtp.googlemail.com:587";

//   Accounts.emailTemplates.siteName = "Harmonic Group";
//   Accounts.emailTemplates.from = 'Harmonic Group Admin <SPOTYL@gmail.com>';
...
//     return 'Verify Account with Harmonic Group';
//   };

//   Accounts.emailTemplates.verifyEmail.html = function(user, url) {
//     return 'Hello,<br><br>Thank You for Signing up on Harmonic Group. Please verify your email address to continue the app use.<br><br>To verify your account email, simply click the link below:<br>'+'\n' + url + '<br><br>Regards,<br>Team Harmonic Group';
//   };
